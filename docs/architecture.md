# npp-rs Architecture

## Two-crate workspace

| Directory | Crate | Purpose |
|-----------|-------|---------|
| `npp-sys/` | `npp-sys` | Bindgen-generated FFI to NPP image domain (`nppi*` symbols) |
| `npp/` | `npp-rs` | Safe wrapper with `CudaImage<T>`, capability traits, operations |

`npp-sys` is a thin generated layer — never edit it directly. `npp-rs` is the
user-facing crate.

## Device management via cudarc

CUDA device management uses [`cudarc`](https://docs.rs/cudarc) 0.9:

```
CudaDevice::new(ordinal)?  →  Arc<CudaDevice>  →  CudaSlice<T>
                                                       ↓
                                             CudaView<'a, T>  (borrowed)
                                             CudaViewMut<'a, T>  (mutable)
```

The `Arc<CudaDevice>` handle must outlive all images created from it (C7
context-lifetime invariant). This is enforced for free by cudarc's internal
`Arc<CudaDevice>` reference on every `CudaSlice<T>`.

## Stream / execution-context model (F8)

Every `CudaImage<T>` carries an `Arc<StreamContext>` that owns the CUDA device
and a forked stream. Operations enqueue asynchronously on the forked stream via
NPP's `_Ctx` API variants, with explicit synchronisation points:

- **Upload → NPP op:** ordered by construction (fork-default-stream inserts an
  implicit wait).
- **NPP op → readback:** requires explicit host fence (`StreamContext::synchronize()`)
  before the device-to-host copy.

`StreamContext` is `!Send + !Sync` (CUDA streams are thread-bound). See
[`docs/stream-context.md`](stream-context.md) for the full ordering contract.

## The `NppPixelType` alphabet

The sealed `NppPixelType` trait marks types that can be used as NPP pixel
elements. It covers ~9 types across the NPP primitive alphabet.

| NPP name | Rust type | Bits |
|----------|-----------|------|
| `8u` | `u8` | 8 |
| `8s` | `i8` | 8 |
| `16u` | `u16` | 16 |
| `16s` | `i16` | 16 |
| `32u` | `u32` | 32 |
| `32s` | `i32` | 32 |
| `32f` | `f32` | 32 |
| `64f` | `f64` | 64 |
| `16f` | `half::f16` | 16 (requires `half` crate — deferred to M2) |

All types are constructible (allocatable + zeroable). Operation capability is
expressed by **separate traits** — an unsupported `(type, op)` pair has no trait
impl, making it a compile-time error.

## Capability-trait model

Operations are modelled as traits implemented on `CudaImage<T>`:

```rust
pub trait Resize: Sized {
    fn resize(&self, dst: &mut Self, inter: ResizeInterpolation) -> Result<(), NppError>;
}

pub trait SwapChannels: Sized {
    fn bgra_to_rgb(&self, dst: &mut Self) -> Result<(), NppError>;
}

pub trait Mean: Sized {
    fn mean(&self) -> Result<Vec<f64>, NppError>;
}

pub trait ConvertTo<Dst: NppPixelType> {
    fn convert(&self, dst: &mut CudaImage<Dst>) -> Result<(), NppError>;
}

pub trait Normalize<Dst: NppPixelType> {
    fn normalize(&self, dst: &mut CudaImage<Dst>) -> Result<(), NppError>;
}
```

F2 expanded Resize/SwapChannels/Mean to cover the full `NppPixelType` alphabet via
macro codegen (see `docs/codegen-architecture.md`). Each trait definition
lives in `imageops.rs`; macro-generated impls live in `*_generated.rs` files
committed to the repo.

**Cross-type operations** (`ConvertTo`, `Normalize`) operate on images of different
pixel types (e.g. `CudaImage<u8>` → `CudaImage<f32>`). These are currently
**hand-written** in `convert_ops.rs` (not codegen-generated); the source and destination
types are distinct trait parameters rather than `Self`.

## `Vec<T>` round-trip

Host ↔ device transfer uses `TryFrom<&CudaImage<T>> for Vec<T>`:

```rust
let host: Vec<u8> = Vec::try_from(&gpu_image)?;
```

There is no `image` crate dependency in the core. Image-rs integration is a
boundary feature (F3 on the roadmap).

## Pointer bridge

The cudarc → NPP pointer bridge follows a fixed pattern:

```rust
let cu_ptr = cudarc::driver::DevicePtr::device_ptr(&buf);
let raw_ptr = (cu_ptr + offset) as *const T;
```

See `docs/spike-cudarc-ptr-bridge.md` for the authoritative pattern used at
every NPP call site.

## Module layout (`npp/src/`)

| Module | Contents |
|--------|----------|
| `cuda.rs` | Device initialization (`CudaDevice::new`) |
| `convert_ops.rs` | Cross-type pixel format conversion (hand-written `u8→f32` ConvertTo + Normalize) |
| `error.rs` | `NppError` enum, `check_status()` helper |
| `image.rs` | `NppPixelType` trait, `CudaImage<T>`, `CudaImageView`, `CudaImageViewMut` |
| `imageops.rs` | Capability trait definitions (`Resize`, `SwapChannels`, `Mean`, `ConvertTo`, `Normalize`) |
| `layout.rs` | `CudaLayout` — packed memory layout description |
| `resize_caps.rs` | GPU-probed `(type, interpolation)` support matrix (committed generated artifact) |
| `resize_generated.rs` | Macro invocations generated by `examples/gen_resize_impls` (committed generated artifact) |
| `resize_macros.rs` | `impl_resize_for!` macro definition (`#[macro_export]`) |
| `resize_ops.rs` | Helper functions (`interpolation_mode`, `mode_supported`) |
| `stream.rs` | `StreamContext` abstraction (forked stream, `NppStreamContext` population, sync points) |
| `swap_channels_generated.rs` | Macro invocations for SwapChannels (all types, C4C3R 4→3 conversion) |
| `swap_channels_macros.rs` | `impl_swap_channels_for!` macro definition |
| `mean_generated.rs` | Macro invocations for Mean (all types, two‑call scratch‑buffer dance) |
| `mean_macros.rs` | `impl_mean_for!` macro definition |
| `test_helpers.rs` | `assert_golden()` helper (GPU‑gated) |

## Deferred boundaries

- **image-rs** (F3): `From`/`TryFrom` edges at the boundary, not in the core.
- **graphynx** (F4): Interop with the author's cudarc-based compute library.
- **Signal ops** (F9): `npps*` symbols are excluded from the bindgen allowlist.

See [docs/roadmap.md](roadmap.md) for the full post-M1 plan.
