/// Macro: generate a `pub(crate)` ROI engine function + `impl Resize for CudaImage<$rust_ty>`.
///
/// # Arguments
///
/// * `$fn_name` — name of the emitted free `pub(crate)` engine function (e.g. `resize_into_8u`).
...
/// 2. An `impl Resize for CudaImage<$rust_ty>` block whose `fn resize` body is a
///    thin wrapper over the engine function.
///
...
/// - `inter` — resize interpolation mode
...
macro_rules! impl_resize_for {
    ($fn_name:ident, $rust_ty:ty, $token:expr, { $($ch:literal => $sym:path),+ $(,)? }) => {
        /// ROI resize engine: resize a source to a destination using raw device pointers.
        ///
        /// This `pub(crate)` function accepts explicit pointers and dimensions so it can
        /// be called from both the owned `Resize` trait impl and from `CudaImageView`-based
        /// sub-image paths. The FFI call sequence is identical to the owned path.
        ///
        ...
        impl Resize for CudaImage<$rust_ty> {
            ...
            ///
            /// This is a thin wrapper over the [`resize_into_`] engine function, which
