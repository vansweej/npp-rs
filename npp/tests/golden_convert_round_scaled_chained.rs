//! `_Ctx` chained-operation smoke test for `ConvertRoundedScaled`.
//!
//! Chains `Resize` (`_Ctx`) → `convert_rounded_scaled` (`_Ctx`) on a single
...
//! separately by `golden_convert_round_scaled_pinned.rs`.
//!
...
//! Run on a GPU host:
//! ```bash
//! nix develop . --command cargo test --features gpu --test golden_convert_round_scaled_chained
//! ```
...
use npp_rs::imageops::{ConvertRoundedScaled, Resize, ResizeInterpolation, RoundMode};
...
/// Golden output for chained resize → ConvertRoundedScaled (Nearest, scale=0), pinned from GPU.
///
...
#[test]
fn chained_resize_then_convert_rounded_scaled_produces_correct_pixels() {
    // ── Setup ──
    ...
    // ── Op 2: ConvertRoundedScaled f32→u8 (Nearest, scale=0) ──
    let mut dst = CudaImage::<u8>::new(ctx.clone(), ch, W / 2, H / 2).expect("allocate dst");
    resized
        .convert_rounded_scaled(&mut dst, RoundMode::Nearest, SCALE)
        .expect("convert_rounded_scaled f32→u8");
    ...
    assert_golden(
        &result,
        EXPECTED,
        "chained_resize_then_convert_rounded_scaled",
    );
}
